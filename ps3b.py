...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    Average_Virus_List = [x / float(numTrials) for x in virusList]
    pylab.plot(Average_Virus_List, label = 'Viruses')
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for drug in activeDrugs:
            if all(self.isResistantTo(drug)) == True and self.maxBirthProb*(1-self.popDensity) >= random.random():
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for virus in self.viruses[:]:
            try:
                self.viruses.append(virus.reproduce(popDensity, self.getPrescription()))
            except NoChildException:
                pass
        return (self.getTotalPop())
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for sim in range(numTrials):
        ...
        for i in range(300):
            ...
            virusList[i] += treated_patient.getTotalPop()
            resisList[i] += treated_patient.getResistPop("guttagonol")
    Avg_VirusList = [x/numTrials for x in virusList]
    Avg_ResisList = [y/numTrials for y in resisList]     
    pylab.plot(Avg_VirusList, label = 'Viruses')
    pylab.plot(Avg_ResisList, label = 'Resistant Viruses')
